...
export default async function NewProjectPage({ searchParams }: NewProjectPageProps) {
    ...
    let parentItem = null;
    if (parentId) {
        parentItem = await prisma.project.findUnique({ where: { id: parentId } });
    }
    ...
    return (
        <div>
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-white">{isTranslation ? `${locale.toUpperCase()} Çevirisi Ekle` : "Yeni Proje"}</h1>
            ...
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-6">
                <ProjectForm mode="create" initialData={isTranslation ? {
                    title: "", slug: "", excerpt: "", content: "",
                    category: parentItem?.category || "", coverImage: parentItem?.coverImage || null,
                    publishedTime: new Date().toISOString().slice(0, 16), isActive: true, locale,
                    parentId: parentId,  // ← parentId eklendi
                } : undefined} />
            </div>
        </div>
    );

...
export function AuditMemberForm({ initialData, mode }: { initialData?: AuditMemberFormData; mode: "create" | "edit" }) {
    const router = useRouter();
    const [loading, setLoading] = useState(false);
    const [formData, setFormData] = useState<AuditMemberFormData>({
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6 max-w-xl">
            ...
            <div className="space-y-2"><Label>Fotoğraf</Label><ImageUpload value={formData.photo} onChange={(url) => setFormData({ ...formData, photo: url })} folder="audit-board" /></div>
            ...
            <div className="flex gap-4 pt-6 border-t border-slate-700">
                ...
                <Button type="button" variant="outline" onClick={() => router.push("/admin/audit-board")}>İptal</Button>
            </div>
        </form>
    );
}
